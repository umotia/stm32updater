...
public final class DfuUtils {
	...
	public static long crc32_table[] = {
	...
	public static long crc32_byte(long accum, int delta) {
			long tmp = accum >> 8;
			int i = (int) ((accum ^ delta) & 0xff);
	        return crc32_table[i] ^ (tmp);
	}
	public static long readUnsignedIntFromBytes(DataInputStream in) throws Exception {
	...
	public static long readIntFromBytes(DataInputStream in) throws Exception {
		byte[] data = new byte[4];
		in.read(data);
		long rtn = 0;
		for (int i=0; i<data.length; i++) {
			rtn = rtn | (0xff & data[i]) << i*8;
		}
		return rtn;
	}
	public static short readShortFromBytes(DataInputStream in) throws Exception {
		byte[] data = new byte[2];
		in.read(data);
		short rtn = (short) (0xff & data[0]);
		rtn = (short) (rtn | (0xff & data[1]) << 8);
		return rtn;
	}

	public static String readStringFromBytes(DataInputStream in, int len) throws Exception {
		byte d[] = new byte[len];
		in.read(d);
		String rtn = "0x";
		for (int i = len-1;i>=0;i--) {
			rtn = rtn+String.format("%02X", d[i]);
		}
		return rtn;
	}
	public static byte[] getBytesfromInt(long in) {
		byte[] data = new byte[4];
		

		for (int i=0; i<data.length; i++) {
			data[i] = (byte) ((in >> i*8) & 0x000000FFL);
		}
		return data;
	}
